...
async function loadTrackCoordinates() {
    ...
    console.log('Loading track coordinates from files...');
    
    for (const [trackType, filename] of Object.entries(trackFiles)) {
        try {
            console.log(`Loading ${trackType} from ${filename}...`);
            const response = await fetch(filename);
            if (response.ok) {
                const text = await response.text();
                const coordinates = parseTrackCoordinates(text);
                console.log(`${trackType}: Loaded ${coordinates.length} coordinates`);
                trackData[trackType] = coordinates;
            } else {
                console.warn(`Could not load ${filename} (${response.status}), using fallback coordinates`);
            }
        } catch (error) {
...
function drawTrackOnCanvas(canvas, coordinates) {
    ...
    // Use Mosport as the reference size, make others 15% smaller
    const trackType = canvas.getAttribute('data-track');
    let scale;
    
    if (trackType === 'mosport') {
        // Mosport size - use 160% of canvas size for maximum size without cutoff
        scale = (Math.min(width, height) * 1.6) / maxRange;
    } else {
        // All other tracks 15% smaller than Mosport
        scale = (Math.min(width, height) * 1.36) / maxRange; // 1.6 * 0.85 = 1.36
    }
